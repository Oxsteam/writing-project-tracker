(function ( $ )  {
	...
	var wptConstants = {
	  ...
	  LOG_START_DATE: 'wt_log_start_date',
	  LOG_END_DATE: 'wt_log_end_date',
	  LOG_WORD_COUNT: 'wt_log_word_count',
	  LOG_NOTES: 'wt_log_notes',
	}
	...
	function clearEditFields()
	{
	 ...
	 function buildLogTable(rowResults)
	 {
		if (rowResults != false)
			var rowString = '';
		...
		function isLastDate(currentLastDate, date)
		{
		...
		function isStartDate(currentStartDate, date)
		{
	 ...
	 function buildStats(rowResults)
	 {
		...
		if (rowResults != null)
		{
			...
			jQuery(rowResults).each(function(index, value) {
				...
				if (index == 0){
				 ...
				 if (isLastDate(currentLastDate, lastDate))
				 	currentLastDate = lastDate;

				if (isStartDate(currentStartDate, startDate))
			...
			jQuery('.start-date-label').text('Date Started: ' + formatDate(currentStartDate));
			jQuery('.last-date-label').text('Date Last Worked: ' + formatDate(currentLastDate));
	  ...
	  jQuery(document).ready(function () {
		...
		function checkEndDate()
		{
			...
			if ((startDate != '') && (endDate != '') && (startDate > endDate)){
				addErrorMessage(jQuery('#end-date-input'), 'Enter End Date after Start Date.');
			}
			else if (endDate == '')
			{
				addErrorMessage(jQuery('#end-date-input'), 'Enter an End Date.');
			}
			else {
				removeErrorMessage(jQuery('#end-date-input'));
			}
		}
		...
		function checkStartDate()
		{
			var startDate = jQuery('#start-date-input').val();
			if (startDate == '')
				addErrorMessage(jQuery('#start-date-input'), 'Enter a Start Date');
			else
			{
				removeErrorMessage(jQuery('#start-date-input'));
				checkEndDate();
			}
		}
		...
		function checkWordCount()
		{
			if (jQuery('.word-count-input').val() == '')
				addErrorMessage(jQuery('.word-count-input'), 'Enter Word Count');
			else
				removeErrorMessage(jQuery('.word-count-input'));
		}
		...
		function addErrorMessage(control, message)
		{
		...
		function validateEdits()
		{
			...
			if (wordCount == '')
			{
				addErrorMessage(jQuery('.word-count-input'), 'Enter a Word Count');
				passed = false;
			}
			else
				removeErrorMessage(jQuery('.word-count-input'));
		...
		function setStatesOnButtonClick(disabled)
		{
