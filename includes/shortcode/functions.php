<?php
function write_track_shortcode_build_project_html( $atts)
{
	$atts = shortcode_atts( array(
		...
		'animate' => false,
		'duration' => 3000,
		'lock_date' => ''), $atts, 'wtdisplayproject');

	if ($atts['projectid'] > 0)
	{
    $current_project = get_post($atts['projectid']);
		$log_ids =  get_post_meta($atts['projectid'], LOG_META_KEY_VALUE);
		$total_est_words = get_post_meta($current_project->ID, '_estwordcount', true);
		$total_words = 0;
    $total_progress = 0;
		...
		foreach ($log_ids as  $meta_key_post=>$meta_value_post)
		{
			if ($meta_value_post != "0"){
				$log_info = get_post_meta($meta_value_post, LOG_META_WORD_COUNT);
				if (count($log_info) == 1)
		...
		if ($total_words > 0)
    {
      ...
      //   'duration' => 3000,
    	...
    	// 	'animate' => false,
    	// 	'lock_date' => '',
      //   'totalProgress' => $total_progress), $atts, 'wtdisplayproject');
	}
