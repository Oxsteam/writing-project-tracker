...
function writing_project_tracker_load(){
		$pg = new WritingProjectTracker();
		$pg->load();
}

add_action('admin_enqueue_scripts', 'Writing_Project_Tracker\writing_project_tracker_load_admin_scripts');
add_action('admin_print_styles', 'Writing_Project_Tracker\writing_project_tracker_load_admin_styles');
add_action('plugins_loaded', 'Writing_Project_Tracker\writing_project_tracker_load');

function writing_project_tracker_load_admin_styles() {
	wp_register_style($handle = 'wt_bootstrap', $src = plugins_url('includes/bootstrap-4.3.1-dist/css/bootstrap.min.css', __FILE__), $deps = array(), $ver = '1.0.0', $media = 'all');
...
function writing_project_tracker_load_admin_scripts() {
	...
	wp_register_script('wt_bootstrap', $src = plugins_url('includes/bootstrap-4.3.1-dist/js/bootstrap.min.js', __FILE__), array('jquery'));
...
add_shortcode('wtdisplayproject', 'write_track_shortcode_build_project_html');


register_activation_hook( __FILE__, 'Writing_Project_Tracker\writing_project_tracker_plugin_activation' );
function writing_project_tracker_plugin_activation() {
...
add_action( 'admin_init','Writing_Project_Tracker\writing_project_tracker_plugin_initialize' );
function writing_project_tracker_plugin_initialize() {
...
register_deactivation_hook(__FILE__, 'Writing_Project_Tracker\writing_project_tracker_plugin_deactivation');
function writing_project_tracker_plugin_deactivation() {
	//do something here later
}


function is_write_tracker_deactivating() {
...
function writing_project_tracker_register_post_type() {
	if (is_write_tracker_deactivating()){
		return;
	}
	...
	register_post_type(LOG_META_KEY_VALUE, $log_args);
	flush_rewrite_rules();


}

function writing_project_tracker_shortcode_callbacks() {
  wp_register_script('wt-progress', $src = plugins_url('includes/js/progressbar.min.js', __FILE__), array('jquery'));
  wp_enqueue_script( 'wt-progress' );

}
add_action('wp_enqueue_scripts', 'Writing_Project_Tracker\writing_project_tracker_shortcode_callbacks');



add_action('init', 'Writing_Project_Tracker\writing_project_tracker_register_project_type_taxonomy');
function writing_project_tracker_register_project_type_taxonomy() {
	$labels = array(
...
add_action('save_post_wtproject', 'Writing_Project_Tracker\writing_project_tracker_save_events_meta'); // save the custom fields
function writing_project_tracker_save_events_meta($post_id) {
    ...
    {
		if (validateDate($_POST["txtgoalcompletedate"], 'Y-m-d'))
